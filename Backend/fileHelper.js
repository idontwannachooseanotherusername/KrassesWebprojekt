...
module.exports.getUploadedFilesAsArray = function(request, handleName, expanded) {
    ...
    if (expanded) {
        for (i = 0; i < data.length; i++) {
            data[i].nameOnly = this.getNameOnly(data[i].name);
            data[i].extension = this.getExtension(data[i].name);
            data[i].isPicture = this.isMimeTypePicture(data[i].mimetype);
            data[i].isWebPicture = this.isMimeTypeWebPicture(data[i].mimetype);
        }
    }
    ...
    for (i = 0; i < handles.length; i++) {
        ...
        for (x = 0; x < data.length; x++) {
            if (expanded) {
                ...
                data[x].extension = this.getExtension(data[x].name);
